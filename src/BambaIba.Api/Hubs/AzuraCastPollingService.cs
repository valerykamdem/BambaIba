...
public class AzuraCastPollingService : BackgroundService, IAzuraCastPollingService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            // C'est normal si l'app s'arrête, on ne log pas comme une erreur fatale
            Console.WriteLine("AzuraCast Polling stopped gracefully.");
        }
        catch (Exception ex)
        {
            // Log les autres erreurs réelles
            Console.WriteLine($"Critical error in polling: {ex.Message}");
        }
    }
